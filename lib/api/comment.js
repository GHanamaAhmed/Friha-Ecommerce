...
const fetchCountComments = async (req) => {
  return await new Promise(async (resolve, reject) => {
    await customAxios
      .get(`/comments/count?type=${req.type}&postId=${req.postId}`)
      .then((res) => resolve(res))
      .catch((err) => reject(err));
  });
};
...
export { fetchReplies, fetchComments, sendComment, removeComment,fetchCountComments };
